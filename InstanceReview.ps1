...
foreach($item in $objs) {
    ...
    sqlcmd -H $sqlsrv -i ComponentsInstalled.sql -U userfordma -P P@ssword135 -s"|" | findstr /v /c:"---" > $foldername\$compFileName
    sqlcmd -H $sqlsrv -i ServerProperties.sql -U userfordma -P P@ssword135 -s"|" | findstr /v /c:"---" > $foldername\$srvFileName
    sqlcmd -H $sqlsrv -i Features.sql -U userfordma -P P@ssword135 -m 1 -s"|" | findstr /v /c:"---" > $foldername\$blockingFeatures
